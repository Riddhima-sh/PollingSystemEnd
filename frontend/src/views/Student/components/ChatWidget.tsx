...
export default function ChatWidget({ studentId, name }: Props) {
  const [open, setOpen] = useState(false);
  const [text, setText] = useState('');
  const dispatch = useDispatch();
  const messages = useSelector((s: RootState) => s.chat.messages);
  const { sendChat } = useRealtimeStudent(studentId, name);

  function onSend(e: FormEvent) {
    e.preventDefault();
    if (!text.trim()) return;
    dispatch(addMessage('student', text.trim(), studentId));
    sendChat(text.trim());
    setText('');
  }
  ...
  return (
    <div className="fixed bottom-4 right-4">
      <button
        className="h-12 px-4 rounded-full bg-gray-900 text-white shadow-lg"
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div className="mt-3 w-80 rounded-2xl border bg-white shadow-xl flex flex-col max-h-96">
          ...
          <form onSubmit={onSend} className="p-3 border-t flex gap-2">
            ...
            />
            <button className="px-4 h-10 rounded-lg bg-gray-900 text-white">Send</button>
          </form>
        </div>
      )}
    </div>
